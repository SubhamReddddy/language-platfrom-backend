...
export const userRegister = catchAsync(async (req, res, next) => {
  ...
  const token = createToken(data._id as mongoose.Types.ObjectId);
  res
    .status(200)
...
export const userLogin = catchAsync(async (req, res, next) => {
  ...
  if (!isUser) throw new ErrorHandler(404, "invalide email or password !");

  const isPassword = bcrypt.compareSync(password, isUser.password);
  if (!isPassword) throw new ErrorHandler(404, "invalide email or password !");
